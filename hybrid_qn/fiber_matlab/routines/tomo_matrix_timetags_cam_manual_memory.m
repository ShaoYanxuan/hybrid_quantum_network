...
tdiff_tmp1 = [];
Counts_for_inset_figure1 = [];
tdiff_tmp2 = [];
Counts_for_inset_figure2 = [];
count_loop =zeros(max(abs(loop_sel))+2, 1);
Counts1 = [];
Counts2 = [];
...
for nfile = 1:Number_of_measurements
    ...
    for i = 1:length(TAGS)
        if TAGS(i, 2) == ChRaman         %loop start
            loop_n = loop_n+1;
            corr_loop = zeros(1,4);  % addded 2021 May 12
             corr_loop2 = zeros(1,4);  % addded 2021 May 12
        ...
        if TAGS(i, 2) == ChCycle           %cycle start
            cycl_num =cycl_num+1;
                 if(cycl_num>1)
                     if(abs_basis>0 && ~isempty(find(loop_sel == abs(loop2_n - loop_n) )) && loop_n>loop2_n   )%  && loop_n<loop2_n  && ~isempty(find(loop_sel == abs(loop2_n - loop_n) ))  )%&& && loop_n<=loop2_n && loop_n<loop2_n  &&   abs(loop2_n - loop_n)>11 && loop_n<loop2_n) %21&&   abs(loop2_n - loop_n)< 30) %&& loop2_n> loop_n % && (loop_n < loop2_n) )%%(loop2_n == loop_n) )%&& loop_n<6 )% && (loop_n < loop2_n) &&(loop2_n - loop_n < 6) )(abs(loop2_n - loop_n) < 1000)
%                     
...
%                          end
                     ...
                     corr_loop = zeros(1,4);corr_loop2 = zeros(1,4);
                       ...
                       if abs(loop2_n - loop_n)>1 % saving wavepackets for cases triggered on phot 1
                            Counts_for_inset_figure1 = [Counts_for_inset_figure1 tdiff_tmp1];
                            Counts_for_inset_figure2 = [Counts_for_inset_figure2 tdiff_tmp2];
                            tdiff_tmp1 = [];
                            tdiff_tmp2 = [];
                       end
...
%                     end
        ...
        if TAGS(i, 2) == CH2 %&& loop_n <21
            ...
            if(ion_basis >= 0)  % check for melting 
               ...
               % clean_timetag(ii,:) = TAGS(i,:);
                tdiff = TAGS(i, 1)-trigger_time;
                 tdiff_tmp2(end+1) = tdiff;
                k2 = k2+1;
                Counts2(k2, nfile) = tdiff;
                
                if( tdiff> gate_start && tdiff < gate_stop )  % CLICK!
                    count_loop(abs(loop_n-loop2_n)+1) = count_loop(abs(loop_n-loop2_n)+1)+1;
...
out.Counts_for_inset_figure1{gate_number} = Counts_for_inset_figure1;
out.Counts_for_inset_figure2{gate_number} = Counts_for_inset_figure2;
