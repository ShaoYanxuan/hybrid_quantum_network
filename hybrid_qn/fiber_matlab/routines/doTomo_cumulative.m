function out = doTomo_cumulative(res, ion)
%clear;clc;close all
 %clear all
%  gg=3;
%  ii = 1;
...
%  out1 = out;
%  clear out
%  for ii = 2:length(res)  % sum of counts for several files/ions
%      clear out
%      load(res{ii})
% %      for jj = 1:length(out1.tomo_matrix1_counts)
% %         out1.tomo_matrix1_counts{jj} = out.tomo_matrix1_counts{jj}; % for each gate
% %      end
%    % out1.tomo_matrix1_counts{ii} = out1.tomo_matrix1_counts{ii-1}+out.tomo_matrix1_counts{ii};
%     out1.tomo_matrix1{ii} = out.tomo_matrix1{ii};
%     out1.tomo_matrix1_counts{ii} = out.tomo_matrix1_counts{ii}; % for each gate
%  end
%  out = out1; 

%% - load the tomo data: probability data Matrix
%load('allresults_2018-12-04_1550_waittimes_vk_1')
% In1 = out;
% clear out
 %load('allresults_2020_04_28_timetags_addressed_entanglement_2_vk_cam.mat')
%load(result)
%  out = summ_data(In1, out);
load(res)
% 
