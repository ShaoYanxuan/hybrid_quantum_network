function out = timetags_cam_2ion_2phot_memory(timetag_root_folder, trics_root_folder, trics_folder_arr,  timetag_files,timetag_bases,  ChRaman, ChRaman2, CH1, CH2, ChCycle, threshold, doppler_threshold, seq_cycles,  gate_start, gate_stop, gate2_shift, gate_number, nloops_param)
% if length(real_cycl_start1) == 1
%     real_cycl_start(nfile) = real_cycl_start1;
% else 
%     real_cycl_start = real_cycl_start1;
% end
%loop_sel = [8:13, 20:25, 32:37, 44:49,56:61]-1;
loop_sel = [10:17, 26:33, 42:49, 58:61]-1;
...
count_loop = zeros(nloops_param, 1);
count_loop2 = zeros(nloops_param, 1);
count_mem_loop = zeros(nloops_param, 1);
count_mem_loop2 = zeros(nloops_param, 1);
count_pair = zeros(nloops_param, nloops_param);
...
for nfile = 1:Number_of_measurements
  if(1)% ~isempty(find(nfile == [1:25,35]))
   % try
    ii = 0;
    clear clean_timetag
    clean_timetag(1,:) = [0, 0];
    ...
    photon_positions = find(kron([1,1,1,1],kron(ph1,ph2)));
        ...
        if TAGS(i, 2) == ChCycle           %cycle start
            cycl_num =cycl_num+1;
                 if(cycl_num>1)
                   ...
                   if (max(phot)>0    && Delt<=185 &&Delt >=0 && loop_n < loop2_n &&  cycl_num >= 20*seq_cycles) %&& mod(fix((Delt-1)/5),2) == 0   %&& loop2_n < loop_n %&& (isempty(find(loop_sel == loop_n-loop2_n)) && isempty(find(loop_sel == loop2_n-loop_n)) 
                      if(sum(phot))>1
                          oups 
                      end
                       %% pair detefcted!:
...
%                       if ~isempty(find(loop_sel == loop_n-loop2_n))    && max(ion_state)>0 %loop_sel-1
...
%                       end
%                       if  ~isempty(find(loop_sel == loop2_n-loop_n))   && max(ion_state)>0
