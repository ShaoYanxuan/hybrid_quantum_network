function out = timetags_2ion_2phot_memory(timetag_root_folder, trics_root_folder, trics_folder_arr,  timetag_files,timetag_bases,  ChRaman, ChRaman2, CH1, CH2, ChCycle, threshold, doppler_threshold, seq_cycles,  gate_start, gate_stop, gate2_shift, gate_number, nloops_param)
        ...
        if TAGS(i, 2) == ChCycle           %cycle start
            cycl_num =cycl_num+1;
                 if(cycl_num>1)
                   ...
                   if max(phot)>0   && abs(loop_n-loop2_n)<=194    %&& (isempty(find(loop_sel == loop_n-loop2_n)) && isempty(find(loop_sel == loop2_n-loop_n)) 
                      if(sum(phot))>1
                          oups 
                      end
                       %% pair detefcted!:
...
%                 end
    ...
    pairs
    clean_timetag(:,1) = clean_timetag(:,1)*1e6; % back to ps for some reason
