...
%PLOT the max-likely states, as 3D bar chart. (abs, parts)
    ...
    zlim([-0.5 0.5])
    set(gca, 'CLim', [0 0.5], 'XTick',[1 2 3 4],'YTick',[1 2 3 4],'XTickLabel',{'HH','HV','VH','VV'} ,'YTickLabel',{'HH','HV','VH','VV'});
    for i = 1:length(AA)
         zdata = abs(AA(i).ZData);
         AA(i).CData = zdata;
         AA(i).FaceColor = 'interp';
        AA(i).FaceAlpha = '0.5'; 
    end
    %t = colorbar('peer', gca, 'Limits', [0, 0.5], 'Visible', 'Off');
    ...
    text( 1,4,-0.35,sprintf(  'F = %.1f +%.1f -%.1f ',(100*FIDE(hh)), 100*out.fidoptSTDp(hh),100*out.fidoptSTDm(hh) )) 
    
   % AA=bar3(abs(out.rhoML_optrot{hh}));
    figure(figure2); 
    subplot(1,A,hh);    
    AAA=bar3(imag(out.rhoML{hh}));
    for i = 1:length(AAA)   AAA(i).FaceAlpha = '0.5'; end
    hold on
    view([-23.84 15.04]);
    zlim([-0.5 0.5])
    if(hh == 1)
...
%     errorbar((1:L )+0.3, out.fidoptM(1:L), out.fidoptSTD(1:L),'dg','MarkerFaceColor' ,'g', 'MarkerSize',1,'LineWidth',2)
    ...
    hold on
    errorbar((1:L )+0.1, CONC(1:L), out.concSTDm(1:L), out.concSTDp(1:L), '^r','MarkerFaceColor' ,'none', 'MarkerSize',10,'LineWidth',2)
    errorbar((1:L )+0.3, FIDE(1:L), out.fidoptSTDm(1:L), out.fidoptSTDp(1:L), 'dg','MarkerFaceColor' ,'none', 'MarkerSize',10,'LineWidth',2)
...
%if length(out.rhoML) == 4
    ...
    hold on
    set(fdistC , 'Position', [0.23, 0.21, 1400, 350])
    fdistF =  figure()
    set(findall(gcf,'-property','FontSize'),'FontSize',14)
    title('Fidelity MC')
    hold on
        
    set(fdistF , 'Position', [0.23, 0.21, 1400, 350])
%     CONC = out.concMd;
%     FIDE = out.fidoptMd;
    ...
    for iii = 1:NN
        ...
        hold on
        if(NN == 4)
            xlabel(['Concurrence for outcome #' num2str(iii)], 'FontName', 'Times New Roman')
        else
        ...
        plot([CONC(iii)-out.concSTDm(iii), CONC(iii)-out.concSTDm(iii)] , sp.YLim, '--', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Median of MC$-\delta_-$')
        plot([CONC(iii)+out.concSTDp(iii), CONC(iii)+out.concSTDp(iii)] , sp.YLim, '--', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Median of MC$+\delta_+$')
        %plot([out.concurrence(iii)-3*out.concSTDm(iii), CONC(iii)-3*out.concSTDm(iii)] , sp.YLim, ':', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Value from data$-3\delta_-$')
        plot([out.concurrence(iii), out.concurrence(iii)] , sp.YLim,  'Color', 'g', 'LineWidth',1.5, 'DisplayName', 'Value from data')
        ...
        if iii ==4
            set(findall(gcf,'-property','FontSize'),'FontSize',14)
            lg = legend('show')
            set(lg, 'Interpreter','latex', 'Position',[0.727142857142857 0.607619046710786 0.114957842145647 0.26857142947969],'FontSize',11)
        end
        ...
        plot([FIDE(iii)-out.fidoptSTDm(iii), FIDE(iii)-out.fidoptSTDm(iii)] , sp.YLim, '--', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Median of MC$-\delta_-$')
        plot([FIDE(iii)+out.fidoptSTDp(iii), FIDE(iii)+out.fidoptSTDp(iii)] , sp.YLim, '--', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Median of MC$+\delta_+$')
        %plot([out.fidopt(iii)-3*out.fidoptSTDm(iii), FIDE(iii)-3*out.fidoptSTDm(iii)] , sp.YLim, ':', 'Color', 'black', 'LineWidth', 1.5, 'DisplayName', 'Value from data$-3\delta_-$')
        plot([out.fidopt(iii), out.fidopt(iii)] , sp.YLim,  'Color', 'g', 'LineWidth',1.5, 'DisplayName', 'Value from data')
        ...
        xlabel(['Fidelity for outcome #' num2str(iii)], 'FontName', 'Times New Roman')
        ...
        if iii == 4
            set(findall(gcf,'-property','FontSize'),'FontSize',14)
            lg = legend('show')
            set(lg, 'Interpreter','latex', 'Position',[0.727142857142857 0.607619046710786 0.114957842145647 0.26857142947969],'FontSize',11) %,'Location', 'northwest' 
        end
