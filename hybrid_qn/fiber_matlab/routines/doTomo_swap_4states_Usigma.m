function out = doTomo_swap_4states_Usigma(res, U, addname)
%combining data for 4 outcomes given 1 unitary affecting Bell states
load(res);
...
%     out.rhoMC{hh}=statetomMCX_projectors(cprb,2,meanphotonnumber,200,samples,ps);%(data, Nqubits, cycleIf s, Niter, N0 mc samples)
    if  isfield(out, 'photon_first')
        out.rhoMC{hh} = MC_victors_cond(counts_4states, countsA_4states, attemptsB_4states,    attempts_4states,     samples,iterations, ps_4states);
    else 
...
if length(last)> 0
    ...
    save([res(1:lastfolder) dir filesep '4state_comb' addname cond res(lastfolder+1:end)],'out');
else
    mkdir(dir)
    save([dir filesep '4state_comb' addname cond res],'out');
end
figure(3)
