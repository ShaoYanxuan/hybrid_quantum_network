function out = timetags_cam_2ion_2phot(timetag_root_folder, trics_root_folder, trics_folder_arr,  timetag_files,timetag_bases,  ChRaman, CH1, CH2, ChCycle, threshold, doppler_threshold, seq_cycles,  gate_start, gate_stop, delay_to_second, gate_number)
...
% %                     end
%                 end
                if( tdiff> gate_start2 && tdiff < gate_stop2 )  % CLICK!
                    triggering_count2 = tdiff;
                    phot2 = [1 0];
                    if(timetag_bases(nfile, 1) > 3)  %VH /HV swap
                        phot2 = ~phot2;
