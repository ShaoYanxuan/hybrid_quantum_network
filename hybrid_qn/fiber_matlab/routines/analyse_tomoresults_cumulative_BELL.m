...
out.fidopt(hh)=mixedfid(rho_base{hh},out.rhoML{hh});   % 12.04.21 change to plot with BELL only for correct observebles


for jj=1:out.samples;
        %out.fidoptmc(hh,jj)=mixedfid(rhotarg,Uopt*out.rhoMC{hh}(:,:,jj)*Uopt');
        out.fidoptmc(hh,jj)=mixedfid(rho_base{hh},out.rhoMC{hh}(:,:,jj)); % 12.04.21 change to plot with BELL only for correct observebles
