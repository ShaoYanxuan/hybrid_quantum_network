...
% attempt right after HOM attemt    
    ...
    if(~exist('coinc_noise_window'))
        coinc_noise_window= noise_window;
    end;
    Rabi_freq = Rabi_freq_strong; % versions compatibility
 %--------------------------------------analysing efficiencies-------and wavepackets---
        
    gate_len_us = gate(2)-gate(1);
    
    
    channeldata1_converted = analyse_counts(TAGS_HOM{nfile}, 2,   histlength  );
    channeldata2_converted = analyse_counts(TAGS_HOM{nfile}, 6, histlength  );
    ...
    [eff_sum(It, nfile), noise_prob_sum(It, nfile)] =efficiency_from_hystogram3(ch1.BinCounts, It+100*nfile, channeldata1_converted.attempts, gate, bin_arr, noise_del, noise_window);
    [eff_long(It, nfile), noise_prob_long(It, nfile)] =efficiency_from_hystogram3(ch1.BinCounts, It+100*nfile, channeldata1_converted.attempts, gate_disting, bin_arr, noise_del, noise_window);
    [eff_short(It, nfile), noise_prob_short(It, nfile)] =efficiency_from_hystogram3(ch1.BinCounts, It+100*nfile,channeldata1_converted.attempts, gate_disting+delay_disting, bin_arr, noise_del, noise_window);
    It = 2;
    figure(It+100*nfile)
    ...
    draw_gate(gate_disting, It+100*nfile, 'red') 
    draw_gate(gate_disting+delay_disting, It+100*nfile, 'red') 
    [eff_sum(It, nfile), noise_prob_sum(It, nfile)] =efficiency_from_hystogram3(ch2.BinCounts, It+100*nfile, channeldata1_converted.attempts, gate, bin_arr, noise_del,noise_window);
    [eff_long(It, nfile), noise_prob_long(It, nfile)] =efficiency_from_hystogram3(ch2.BinCounts, It+100*nfile, channeldata1_converted.attempts, gate_disting, bin_arr, noise_del, noise_window);
    [eff_short(It, nfile), noise_prob_short(It, nfile)] =efficiency_from_hystogram3(ch2.BinCounts, It+100*nfile, channeldata1_converted.attempts, gate_disting+delay_disting, bin_arr, noise_del, noise_window);

     if(nfile == MaxFile)
        eff_long
        eff_short
        attempts
        noise_short_path = noise_prob_short/gate_len_us*1e6
        noise_long_path =noise_prob_long/gate_len_us*1e6
    end;
    
    bin_ar = bin_arr(1:length(bin_arr)-1);
    clear tosave
    tosave(:,1) = bin_ar;
    tosave(:, 2)= ch1.BinCounts/channeldata1_converted.attempts-subtract_noise*noise_prob_short(1, nfile)/gate_length;
    tosave(:, 3) = ch1.BinCounts;
    tosave(:, 4)= ch2.BinCounts/channeldata1_converted.attempts-subtract_noise*noise_prob_short(2, nfile)/gate_length;
    ...
    if(nfile ==1)
        averaged_wp(:,1) =  bin_ar;
         averaged_wp(:, 2) = tosave(:, 2);
         averaged_wp(:, 3) = ch1.BinCounts;
         averaged_wp(:, 4) = tosave(:, 4);
        averaged_wp(:, 5) =  ch2.BinCounts;
    else
        averaged_wp(:, 2) = averaged_wp(:, 2)+tosave(:, 2);
        averaged_wp(:, 3) = averaged_wp(:, 3)+ch1.BinCounts(:);
        averaged_wp(:, 4) = averaged_wp(:, 4)+tosave(:, 4);
        averaged_wp(:, 5) = averaged_wp(:, 5)+ch2.BinCounts(:);
    end
    if(nfile == MaxFile)
        averaged_wp(:, 2) = averaged_wp(:, 2)/length(nfile);
        averaged_wp(:, 4) = averaged_wp(:, 4)/length(nfile);
...
%     xlim(gate_disting_big)
   ...
   %then we do histograms with different binning
    ...
    [coinc_prob(1), noise_C_prob(1)] = efficiency_from_hystogram3( HOM1, 100*nfile+8, Coinc_HOM1.attempts,  coinc_gate , bin_arr, 10, coinc_noise_window);
    if(noise_C_prob(1)*Coinc_HOM1.attempts)
        noise_C_prob_err = 1/sqrt(noise_C_prob(1)*Coinc_HOM1.attempts/(coinc_gate(2)-coinc_gate(1))*coinc_noise_window);
    else
        noise_C_prob_err = 0;
    end;
        
    for MC_i = 1: MC_samples
        [coinc_r_prob( MC_i,1), noise_r_C_prob( MC_i, 1)] = efficiency_from_hystogram3( HOM1_r( MC_i,:), 100*nfile+8, Coinc_HOM1.attempts,  coinc_gate , bin_arr, 10, coinc_noise_window);
    end
    ...
    [coinc_prob(2), noise_C_prob(2)] = efficiency_from_hystogram3(HOM2, 100*nfile+7, Coinc_P1.attempts,  coinc_gate +delay_disting, bin_arr, 10,coinc_noise_window );
    [coinc_prob_t, noise_C_prob_t] = efficiency_from_hystogram3(HOM2, 100*nfile+7, Coinc_P1.attempts,  coinc_gate -delay_disting, bin_arr, 10,coinc_noise_window);
    ...
    Total_attemptsb = 0;
    binwidths = [0.25 0.5];    %select desired bins set here 
