function [N1, N2, N12, gg, delays] = g2(TAGS, maxdelay, gate, dt, window, CH1, CH2, trash)
N =length(TAGS) ;
...
for i = 1:(length(delay_arr)-1)
 ...
 %if(delay_arr(i) <= gate(1)+window && delay_arr(i+1)>gate(1)+window)
        gate_stop_bin = i-zer
    end;
...
for i = 1:(length(delay_arr)-gate_stop_bin-1)
    %N1 = sum(ch1_hist(gate_start_bin+zer:gate_stop_bin+zer))/Coinc.attempts/gate_length;  %!!!!!!!
